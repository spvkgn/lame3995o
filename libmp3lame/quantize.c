...
VBR_new_prepare(lame_internal_flags * gfc,
                ...
                int* max_resv, int *maximum_framebits)
{
    SessionConfig_t const *const cfg = &gfc->cfg;
    EncResult_t *const eov = &gfc->ov_enc;
...
void comp_targetBits(lame_internal_flags * gfc) {
	/* sets gfc->targetBits[gr][ch] and gfc->hasShortBlock, gfc->hasStartShortBlock, gfc->hasShortShortBlock, gfc->hasShortStopBlock */

    SessionConfig_t const *const cfg = &gfc->cfg;
    int const ngr = cfg->mode_gr;
    int gr;
	int	targetBits, targetBits_energypart, targetBitsFrame, targetBits_max, maximum_framebits;
	int delta;
	int targetBits_norm = 0;
	int targetBits_stop = 0;
	int targetBits_short = 0;
	int targetBits_start = 0;
	int tb = 0;
	FLOAT attack_factor = gfc->fr_attack_factor;
	FLOAT tonality_factor = gfc->fr_tonality_factor;

	/* console_printf("\ntonality_factor: %6.6f, attack_factor: %6.6f", tonality_factor, attack_factor); */
	attack_factor = (attack_factor - 10.0f) / 35.0f;
	if (attack_factor > 1.0f)
		attack_factor = 1.0f;
	if (attack_factor < 0.0f)
		attack_factor = 0.0f;
	attack_factor = attack_factor * attack_factor;
	/* console_printf("\nattack_factor: %6.6f", attack_factor); */ 
	targetBits_norm = gfc->targetBits_ori_norm_min + attack_factor * (gfc->targetBits_ori_norm_att - gfc->targetBits_ori_norm_min);
	targetBits_start = gfc->targetBits_ori_start_min + attack_factor * (gfc->targetBits_ori_start_att - gfc->targetBits_ori_start_min);
	targetBits_stop = gfc->targetBits_ori_stop_min + attack_factor * (gfc->targetBits_ori_stop_att - gfc->targetBits_ori_stop_min);
	targetBits_short = gfc->targetBits_ori_short_min + attack_factor * (gfc->targetBits_ori_short_att - gfc->targetBits_ori_short_min);

	if (gfc->Qx != -9999) {
		/* console_printf("\ntonality_factor: %6.6f, attack_factor: %6.6f", tonality_factor, attack_factor); */
		tb = gfc->targetBits_ori_norm_min + tonality_factor * (gfc->targetBits_ori_norm_ton - gfc->targetBits_ori_norm_min);
		if (tb > targetBits_norm)
			targetBits_norm = tb;
		tb = gfc->targetBits_ori_start_min + tonality_factor * (gfc->targetBits_ori_start_ton - gfc->targetBits_ori_start_min);
		if (tb > targetBits_start)
			targetBits_start = tb;
		tb = gfc->targetBits_ori_stop_min + tonality_factor * (gfc->targetBits_ori_stop_ton - gfc->targetBits_ori_stop_min);
		if (tb > targetBits_stop)
			targetBits_stop = tb;
		tb = gfc->targetBits_ori_short_min + tonality_factor * (gfc->targetBits_ori_short_ton - gfc->targetBits_ori_short_min);
		if (tb > targetBits_short)
			targetBits_short = tb;
	}

	maximum_framebits = 8064 + gfc->last_Resv;
	gfc->hasShortBlock = 0;
	gfc->hasStartShortBlock = 0;
	gfc->hasShortShortBlock = 0;
	gfc->hasShortStopBlock = 0;

	if  ( (gfc->l3_side.tt[0][0].block_type == SHORT_TYPE) || (gfc->l3_side.tt[1][0].block_type == SHORT_TYPE) ) { /* short block case */

		gfc->hasShortBlock = 1;

		if  (gfc->l3_side.tt[0][0].block_type == START_TYPE) {
			gfc->hasStartShortBlock = 1;
		}
		else if (gfc->l3_side.tt[1][0].block_type == STOP_TYPE) {
			gfc->hasShortStopBlock = 1;
		}
		else {
			gfc->hasShortShortBlock = 1;
		}
	...
	targetBitsFrame = 0;
    for (gr = 0; gr < ngr; ++gr) {
		if (gfc->energy[gr] > 0.00000005) {
			float energyRef = 2.5;
			if (gfc->energy[gr] < energyRef)
				targetBits_energypart = 100.0 * log( gfc->energy[gr] / energyRef ); /* energy dependent correction */
			else
				targetBits_energypart = 0.0;
		}
		else
			targetBits_energypart = - 1770;
		/* console_printf("\ntargetBits_energypart: %i\n", targetBits_energypart); */
		if (gfc->l3_side.tt[gr][0].block_type == SHORT_TYPE) { /* short block case */
			gfc->snr_increase_max[gr] = gfc->snrincmax_short;
			targetBits = targetBits_short  + targetBits_energypart;
		}
		else if (gfc->l3_side.tt[gr][0].block_type == START_TYPE) { /* start block case */
			gfc->snr_increase_max[gr] = gfc->snrincmax_start;
			targetBits = targetBits_start + targetBits_energypart;
		}
		else if (gfc->l3_side.tt[gr][0].block_type == STOP_TYPE) { /* stop block case */
			gfc->snr_increase_max[gr] = gfc->snrincmax_stop;
			targetBits = targetBits_stop + targetBits_energypart;
			if (targetBits > gfc->targetBits_ori_norm_max )
				targetBits = gfc->targetBits_ori_norm_max;
		}
		else { /* norm block case */
			gfc->snr_increase_max[gr] = gfc->snrincmax_norm;
			targetBits = targetBits_norm + targetBits_energypart;
			if (targetBits > gfc->targetBits_ori_norm_max )
				targetBits = gfc->targetBits_ori_norm_max;
		}
		if (targetBits < 0)
			targetBits = 0;
		if ( targetBits > ( MAX_BITS_PER_GRANULE - 2 * gfc->targetBits_ori_tolerance ) )
			targetBits = MAX_BITS_PER_GRANULE - 2 * gfc->targetBits_ori_tolerance;
		gfc->targetBits[gr] = targetBits;
		targetBitsFrame += targetBits;
	}

	targetBits_max = maximum_framebits - gfc->size_bitreservoir; /* maximum possible targetBits such that size_bitreservoir bits are kept in bit reservoir */

	if (!gfc->hasShortBlock) {
		if (targetBitsFrame > targetBits_max) {
			int tf = 0;
			delta = targetBitsFrame - targetBits_max;
			targetBits_max +=  62 * delta / 100;
		    for (gr = 0; gr < ngr; ++gr) {
				gfc->targetBits[gr] *= targetBits_max;
				gfc->targetBits[gr] /= targetBitsFrame;
				tf += gfc->targetBits[gr];
			}
			targetBitsFrame = tf;
		}
	}

	if (gfc->insane_factor != 0.0) {
		if ( ( targetBitsFrame < (targetBits_max - 150) ) && (gfc->l3_side.tt[1][0].block_type != START_TYPE) && (targetBitsFrame != 0) ) {
			int tf = 0;
			for (gr = 0; gr < ngr; ++gr) {
				targetBits = gfc->targetBits[gr] * (targetBits_max - 150) / targetBitsFrame;
				targetBits = gfc->targetBits[gr] + gfc->insane_factor * ( targetBits - gfc->targetBits[gr]);
				if ( targetBits > ( MAX_BITS_PER_GRANULE - 2 * gfc->targetBits_ori_tolerance ) )
					targetBits = MAX_BITS_PER_GRANULE - 2 * gfc->targetBits_ori_tolerance;
				gfc->targetBits[gr] = targetBits;
				tf += targetBits;
	...
	targetBits_max = maximum_framebits - 2 * gfc->targetBits_ori_tolerance;
	if (targetBitsFrame > targetBits_max) {
		if ( (gfc->hasShortShortBlock) && (!gfc->lastFrameHasShortBlock) && (gfc->targetBits[0] != 0)  && (gfc->targetBits[1] != 0) ) {
			double	const OutOfDataRelative[7] =  {	0.0,  0.16667,  0.2727,  0.4,  0.5556,  0.75,  1.0 };
			double	const participation[7] =	  {	0.25, 0.3,      0.38,    0.42, 0.45,    0.47,  0.5 };
			double r, share, f;
			int		i, prev_i;
			r = (double) (targetBitsFrame - targetBits_max) / (double) targetBits_max;
			for (i=0; i<=9; i++) {
				if (r >= OutOfDataRelative[i])
					break;
			}
			if (i == 0) {
				prev_i = 0;
				...
				prev_i = i - 1;
				share = (OutOfDataRelative[i] - r) / (OutOfDataRelative[i] - OutOfDataRelative[prev_i]);
			}
			f = participation[i] + share * (participation[prev_i] - participation[i]);
			f *= 2.0 * gfc->targetBits[1] / ( gfc->targetBits[0] + gfc->targetBits[1] ); /* increase f if targetBits[1] < targetBits[0] */
			delta = targetBitsFrame - targetBits_max;
			gfc->targetBits[0] -= f * delta;
			if (gfc->targetBits[0] < 0)
				gfc->targetBits[0] = 0;
			gfc->targetBits[1] = targetBits_max - gfc->targetBits[0];
			if (gfc->targetBits[1] < 0)
				gfc->targetBits[1] = 0;
			return;
		}
		if ( (gfc->hasShortStopBlock) && (gfc->targetBits[0] != 0) ) {
			double stop_factor = 1.0 - (1.0 - 4300.0/targetBits_stop) * 2.0 * (targetBitsFrame / targetBits_max - 1.0);
			gfc->targetBits[0] *= ( 1.0 + gfc->targetBits[1] / gfc->targetBits[0] * ( 1.0 - stop_factor) ) * targetBits_max;
			gfc->targetBits[0] /= targetBitsFrame;
			gfc->targetBits[1] *= stop_factor * targetBits_max;
			gfc->targetBits[1] /= targetBitsFrame;
			return;
		}
	    for (gr = 0; gr < ngr; ++gr) {
			gfc->targetBits[gr] *= targetBits_max;
...
void comp_snrinc(lame_internal_flags * gfc) {
	...
	for (gr = 0; gr < ngr; ++gr) {
		gfc->snrinc_lo[gr] = -9999.99;
		gfc->snrinc_hi[gr] = +9999.99;
	}

	if ( (gfc->lastFrameHasShortBlock) && (gfc->hasShortBlock) ) {
		gfc->ns_snr_increase[0] = 0.0;
		gfc->ns_snr_increase[1] = 0.0;
		if (gfc->hasShortStopBlock) {
			if (gfc->last_Resv < 300)
				return;
			if (gfc->targetBits[0] != 0)
				gfc->ns_snr_increase[0] = 3.0;
			if (gfc->ns_snr_increase[0] > gfc->snr_increase_max[0])
				gfc->ns_snr_increase[0] = gfc->snr_increase_max[0];
			if (gfc->targetBits[1] != 0)
				gfc->ns_snr_increase[1] = 7.0;
			if (gfc->ns_snr_increase[1] > gfc->snr_increase_max[1])
				gfc->ns_snr_increase[1] = gfc->snr_increase_max[1];
			return;
		}
		if (gfc->hasShortShortBlock) {
			if (gfc->targetBits[0] != 0)
				gfc->ns_snr_increase[0] = 0.0;
			if (gfc->ns_snr_increase[0] > gfc->snr_increase_max[0])
				gfc->ns_snr_increase[0] = gfc->snr_increase_max[0];
		...
		if (gfc->hasStartShortBlock) {
			if (gfc->last_Resv < 1000)
				return;
			if (gfc->targetBits[0] != 0)
				gfc->ns_snr_increase[0] = 1.0;
			if (gfc->ns_snr_increase[0] > gfc->snr_increase_max[0])
				gfc->ns_snr_increase[0] = gfc->snr_increase_max[0];
			if (gfc->targetBits[1] != 0)
				gfc->ns_snr_increase[1] = 2.0;
			if (gfc->ns_snr_increase[1] > gfc->snr_increase_max[1])
				gfc->ns_snr_increase[1] = gfc->snr_increase_max[1];
			return;
		}
	...
	if (gfc->l3_side.tt[0][0].block_type == STOP_TYPE) {
		gfc->ns_snr_increase[0] = 0.0;
		gfc->ns_snr_increase[1] = 0.0;
		if (gfc->targetBits[0] != 0)
			gfc->ns_snr_increase[0] = gfc->lastsnrinc_stopxxx[0];
		if (gfc->ns_snr_increase[0] > gfc->snr_increase_max[0])
			gfc->ns_snr_increase[0] = gfc->snr_increase_max[0];
		if (gfc->targetBits[1] != 0)
			gfc->ns_snr_increase[1] = gfc->lastsnrinc_stopxxx[1];
		if (gfc->ns_snr_increase[1] > gfc->snr_increase_max[1])
			gfc->ns_snr_increase[1] = gfc->snr_increase_max[1];
		return;
	}
	...
	if (gfc->hasStartShortBlock) {
		...
		gfc->ns_snr_increase[1] = 0.0;
		if (gfc->targetBits[0] != 0)
			gfc->ns_snr_increase[0] = gfc->lastsnrinc_fr1_startshort[0];
		if (gfc->ns_snr_increase[0] > gfc->snr_increase_max[0])
			gfc->ns_snr_increase[0] = gfc->snr_increase_max[0];
		if (gfc->targetBits[1] != 0)
			gfc->ns_snr_increase[1] = gfc->lastsnrinc_fr1_startshort[1];
		if (gfc->ns_snr_increase[1] > gfc->snr_increase_max[1])
			gfc->ns_snr_increase[1] = gfc->snr_increase_max[1];
		return;
	}
	...
	if (gfc->hasShortShortBlock) {
		...
		gfc->ns_snr_increase[1] = 0.0;
		if (gfc->targetBits[0] != 0)
			gfc->ns_snr_increase[0] = gfc->lastsnrinc_fr1_shortshort[0];
		if (gfc->ns_snr_increase[0] > gfc->snr_increase_max[0])
			gfc->ns_snr_increase[0] = gfc->snr_increase_max[0];
		if (gfc->targetBits[1] != 0)
			gfc->ns_snr_increase[1] = gfc->lastsnrinc_fr1_shortshort[1];
		if (gfc->ns_snr_increase[1] > gfc->ns_snr_increase[0])
			gfc->ns_snr_increase[1] = gfc->ns_snr_increase[0];
		if (gfc->ns_snr_increase[1] > gfc->snr_increase_max[1])
			gfc->ns_snr_increase[1] = gfc->snr_increase_max[1];
		return;
	}
	...
	if (gfc->hasShortStopBlock) {
		...
		if (gfc->last_Resv < 300)
			return;
		if (gfc->targetBits[0] != 0)
			gfc->ns_snr_increase[0] = gfc->lastsnrinc_fr1_shortstop[0];
		if (gfc->ns_snr_increase[0] > gfc->snr_increase_max[0])
			gfc->ns_snr_increase[0] = gfc->snr_increase_max[0];
		if (gfc->targetBits[1] != 0)
			gfc->ns_snr_increase[1] = gfc->lastsnrinc_fr1_shortstop[1];
		if (gfc->ns_snr_increase[1] > gfc->snr_increase_max[1])
			gfc->ns_snr_increase[1] = gfc->snr_increase_max[1];
		return;
	}
	...
	for (gr = 0; gr < ngr; ++gr) {
		...
		if (gfc->targetBits[0] == 0)
			gfc->ns_snr_increase[gr] = 0.0;
		else
			gfc->ns_snr_increase[gr] =  gfc->last_Resv * gfc->snrinc_lastnorm / 4088.0;
...
void comp_snrinc_dependence(lame_internal_flags * gfc, int nr_iteration, int savcountBadFrames) {

    SessionConfig_t const *const cfg = &gfc->cfg;
    int const ngr = cfg->mode_gr;
    int gr;
	double snrinc_dependence;

	for (gr = 0; gr < ngr; ++gr) {

		if ( gfc->countBadFrames == (savcountBadFrames) ) { /* a well encodable frame */
			if (gfc->usedBits[gr] >= gfc->targetBits[gr]) { /* we arrived at or above targetBits */
				if (gfc->ns_snr_increase[gr] < gfc->snrinc_hi[gr]) {
					gfc->snrinc_hi[gr] = gfc->ns_snr_increase[gr];
					gfc->bits_hi[gr] = gfc->usedBits[gr];
				}
			}
			if (gfc->usedBits[gr] <= gfc->targetBits[gr]) { /* we are at or below targetBits */
				if (gfc->ns_snr_increase[gr] > gfc->snrinc_lo[gr]) {
					gfc->snrinc_lo[gr] = gfc->ns_snr_increase[gr];
					gfc->bits_lo[gr] = gfc->usedBits[gr];
				}		
			}		
		}
			
		if (nr_iteration == 0) {
		   if (gfc->l3_side.tt[gr][0].block_type == NORM_TYPE)
			   gfc->snrinc_dependence[gr] = 1.1 * gfc->snrincdep_lastnorm;
			   if (gfc->snrinc_dependence[gr] > 0.013)
				   gfc->snrinc_dependence[gr] = 0.013;
		   else
			   gfc->snrinc_dependence[gr] = 0.013; /* force a rather big step in order to get at both sides of targetbits with the first two iteration steps with a high probability */
		   gfc->snrinc_depref[gr] = gfc->ns_snr_increase[gr];
		   gfc->bits_depref[gr] = gfc->usedBits[gr];
		   continue;
		}

		/* console_printf("\n\n snrinc_lo[gr]: %.5f, gfc->snrinc_hi[gr]: %.5f", gfc->snrinc_lo[gr], gfc->snrinc_hi[gr]); */
		if ( (gfc->snrinc_lo[gr] != -9999.99) && (gfc->snrinc_hi[gr] != +9999.99) && (gfc->bits_lo[gr] != gfc->bits_hi[gr]) && ( (gfc->bits_hi[gr] - gfc->bits_lo[gr]) < 3.0 * abs(gfc->last_usedBits[gr] - gfc->usedBits[gr]) ) ) {
			if (  (  (gfc->snrinc_hi[gr] - gfc->snrinc_lo[gr]) * (gfc->bits_hi[gr] - gfc->bits_lo[gr]) ) > 0  ) { /* compute gfc->snrinc_dependence only in plausible situations */
				double min_snrinc_dep, max_snrinc_dep;
				snrinc_dependence = (gfc->snrinc_hi[gr] - gfc->snrinc_lo[gr]) / (gfc->bits_hi[gr] - gfc->bits_lo[gr]);
				min_snrinc_dep = 0.00001;
				if (snrinc_dependence < min_snrinc_dep)
					snrinc_dependence = min_snrinc_dep;
				max_snrinc_dep = 0.013;
				if (snrinc_dependence > max_snrinc_dep)
					snrinc_dependence = max_snrinc_dep;
				gfc->snrinc_dependence[gr] = snrinc_dependence;
				gfc->snrinc_depref[gr] = gfc->snrinc_lo[gr];
				gfc->bits_depref[gr] = gfc->bits_lo[gr]; 
				continue;
			}
		}

		if (gfc->usedBits[gr] != gfc->last_usedBits[gr]) {
			if  (  ( ( gfc->last_snrinc[gr] - gfc->ns_snr_increase[gr] ) * ( gfc->last_usedBits[gr] - gfc->usedBits[gr] ) ) > 0  ) { /* compute gfc->snrinc_dependence only in plausible situations */
				double min_snrinc_dep, max_snrinc_dep;
				snrinc_dependence = ( gfc->last_snrinc[gr] - gfc->ns_snr_increase[gr] ) / ( gfc->last_usedBits[gr] - gfc->usedBits[gr] );
				min_snrinc_dep = 0.00001;
				if (snrinc_dependence < min_snrinc_dep)
					snrinc_dependence = min_snrinc_dep;
				max_snrinc_dep = 0.013;
...
int break_on_snrinc_control(lame_internal_flags * gfc, int analog_silence, int savcountBadFrames, int break_snrinc_iteration[2], double snrinc_hiLimit[2], double snrinc_loLimit[2]) {

    SessionConfig_t const *const cfg = &gfc->cfg;
    int const ngr = cfg->mode_gr;
    int gr;
	int count_break_on_granules = 0;
	double	delta, snrinc;

	if (analog_silence)
		return 1; /* break the snrinc iteration */

	for (gr = 0; gr < ngr; ++gr) {

		if (break_snrinc_iteration[gr]) {
			/* console_printf("\n>>>gr: %u, ns_snr_increase[gr]: %.3f, usedBits[gr]: %u, gfc->targetBits[gr]: %u, snrinc_dep[gr]: %.5f DONE", gr, gfc->ns_snr_increase[gr], gfc->usedBits[gr], gfc->targetBits[gr], gfc->snrinc_dependence[gr]); */
			count_break_on_granules++;
			continue;
		}

		if ( (gfc->hasShortShortBlock) && (gr == 1) & ( break_snrinc_iteration[0] == 1 ) ) { /* for shortshort Frames: don't let SNR increase for the 2nd granule be significantly larger than that of the 1st */
			if (snrinc_hiLimit[gr] > gfc->ns_snr_increase[0] + 1.5)
				snrinc_hiLimit[gr] = gfc->ns_snr_increase[0] + 1.5;
			if (snrinc_loLimit[gr] > gfc->ns_snr_increase[0] + 1.5)
				snrinc_loLimit[gr] = gfc->ns_snr_increase[0] + 1.5;
		}

		if ( gfc->countBadFrames != (savcountBadFrames) ) { /* a poorly encodabe frame => go for a well encodable frame */
			/* console_printf("\n>>>gr: %u, ns_snr_increase[gr]: %.3f, usedBits[gr]: %u, gfc->targetBits[gr]: %u, snrinc_dep[gr]: %.5f, snrinc_loLimit[gr]: %.5f, snrinc_hiLimit[gr]: %.5f BAD FRAME", gr, gfc->ns_snr_increase[gr], gfc->usedBits[gr], gfc->targetBits[gr], gfc->snrinc_dependence[gr], snrinc_loLimit[gr], snrinc_hiLimit[gr]); */
			if ( gfc->ns_snr_increase[gr] < (snrinc_loLimit[gr] + 1.0) ) {
				if (gfc->ns_snr_increase[gr] == snrinc_loLimit[gr]) {
					break_snrinc_iteration[gr] = 1;
					count_break_on_granules++;
					continue;
				}
				gfc->ns_snr_increase[gr] = snrinc_loLimit[gr];
				break_snrinc_iteration[gr] = 1;
				continue;
			}
	 		if ( (gfc->lastFrameHasShortBlock) && (gfc->hasShortShortBlock) ) {
				gfc->ns_snr_increase[gr] = snrinc_loLimit[gr];
				break_snrinc_iteration[gr] = 1;
				continue;
			}
			if ( (gfc->ns_snr_increase[gr] < snrinc_hiLimit[gr]) && (gfc->usedBits[gr] > gfc->targetBits[gr]) && ( (gfc->ns_snr_increase[gr] - 0.2) < (snrinc_hiLimit[gr] - 0.5) ) )
				snrinc_hiLimit[gr] = gfc->ns_snr_increase[gr] - 0.2;
			else
				snrinc_hiLimit[gr] = snrinc_hiLimit[gr] - 0.5;
			if (gfc->usedBits[gr] == 0)
				delta = 0.38 * (gfc->ns_snr_increase[gr] - snrinc_loLimit[gr]);
			else {
				delta = gfc->targetBits[gr]/gfc->usedBits[gr] - 0.5;
				if (delta < 0.2)
					delta = 0.2;
				if (delta > 0.8)
					delta = 0.8;
				delta = delta * (gfc->ns_snr_increase[gr] - snrinc_loLimit[gr]);
			}
			if (delta < 0.15) {
				gfc->ns_snr_increase[gr] = snrinc_loLimit[gr];
				break_snrinc_iteration[gr] = 1;
			...
			gfc->ns_snr_increase[gr] = snrinc_loLimit[gr] + delta;
			continue;
		}

		/* frame could be encoded well: */

		/* console_printf("\n>>>gr: %u, ns_snr_increase[gr]: %.3f, usedBits[gr]: %u, gfc->targetBits[gr]: %u, snrinc_dep[gr]: %.5f, snrinc_loLimit[gr]: %.5f, snrinc_hiLimit[gr]: %.5f GOOD FRAME", gr, gfc->ns_snr_increase[gr], gfc->usedBits[gr], gfc->targetBits[gr], gfc->snrinc_dependence[gr], snrinc_loLimit[gr], snrinc_hiLimit[gr]); */

		if (gfc->usedBits[gr] >= gfc->targetBits[gr]) { /* we arrived at or above targetBits */
			if ( gfc->usedBits[gr] <= (gfc->targetBits[gr] + 1.5 * gfc->targetBits_ori_tolerance) ) {
				break_snrinc_iteration[gr] = 1;
				count_break_on_granules++;
				continue;
			}
			...
			if (gfc->ns_snr_increase[gr] < snrinc_hiLimit[gr])
				snrinc_hiLimit[gr] = gfc->ns_snr_increase[gr];
			if (gfc->snrinc_lo[gr] == -9999.99) {
				delta = ( gfc->usedBits[gr] - gfc->targetBits[gr] ) / gfc->ns_snr_increase[gr];
				if (delta > 100) {
					gfc->ns_snr_increase[gr] = 0.0;
					continue;
				}
			}
			delta = gfc->snrinc_dependence[gr] * ( gfc->targetBits[gr] - gfc->bits_depref[gr] );
			if (delta >= 0) {
				if (delta < 0.25)
					delta = 0.25;
				if (delta > 7.0)
			...
			snrinc = gfc->snrinc_depref[gr] + delta;
			if ( (snrinc >= snrinc_hiLimit[gr]) || (snrinc <= snrinc_loLimit[gr]) ) {
				delta = 0.38 * (snrinc_hiLimit[gr] - snrinc_loLimit[gr]);
				if (delta < 0.15) {
					break_snrinc_iteration[gr] = 1;
					count_break_on_granules++;
					continue;
				}
				gfc->ns_snr_increase[gr] = snrinc_hiLimit[gr] - delta;
				continue;
			}
			gfc->ns_snr_increase[gr] = snrinc;
			continue;
		}

		/* we are below targetBits: */

		if ( gfc->usedBits[gr] >= (gfc->targetBits[gr] - gfc->targetBits_ori_tolerance) ) {
			break_snrinc_iteration[gr] = 1;
			count_break_on_granules++;
			continue;
		}
		if (gfc->ns_snr_increase[gr] == gfc->snr_increase_max[gr]) {
			break_snrinc_iteration[gr] = 1;
			count_break_on_granules++;
			continue;
		}
		if (gfc->ns_snr_increase[gr] > snrinc_loLimit[gr])
			snrinc_loLimit[gr] = gfc->ns_snr_increase[gr];
		delta = gfc->snrinc_dependence[gr] * ( gfc->targetBits[gr] - gfc->bits_depref[gr] );
		if (delta < 0.5)
			delta = 0.5;
		if (delta > 7.0)
			delta = 7.0;
		snrinc = gfc->snrinc_depref[gr] + delta;
		/* console_printf("\n>>gfc->snrinc_depref[gr]: %.5f, delta: %.5f, snrinc: %.5f", gfc->snrinc_depref[gr], delta, snrinc); */
		if ( snrinc >= snrinc_hiLimit[gr] ) {
			delta = 0.62 * (snrinc_hiLimit[gr] - gfc->snrinc_depref[gr]);
			if (delta < 0.25) {
				break_snrinc_iteration[gr] = 1;
				count_break_on_granules++;
				continue;
			}
			gfc->ns_snr_increase[gr] = gfc->snrinc_depref[gr] + delta;
			continue;
		}
		gfc->ns_snr_increase[gr] = snrinc;
		continue;
	...
	if (count_break_on_granules == ngr)
		return 1;
	else
		return 0;
                       ...
                       const FLOAT ms_ener_ratio[2], const III_psy_ratio ratio[2][2])
{
	...
	III_side_info_t sav_l3_side;
	int		savcountBadFrames;
	int		done, nr_iteration;
	double	snrinc_hiLimit[2], snrinc_loLimit[2];
	int		break_snrinc_iteration[2]; /* 1: stop the snrinc iteration for granule 0 resp. 1 */
    int     bits;
	double	sav_snrinc[2];
	int		maximum_framebits, max_fr;
	...
	savcountBadFrames = gfc->countBadFrames;
	gfc->countFrames += 1;
	nr_iteration = 0;

	memcpy(&sav_l3_side, &gfc->l3_side, sizeof(III_side_info_t)); /* sav_l3_side = gfc->l3_side; */

	(void) comp_targetBits(gfc);
	gfc->bitreservoir_target = gfc->size_bitreservoir;
	comp_snrinc(gfc);

    for (gr = 0; gr < cfg->mode_gr; gr++) {
		snrinc_hiLimit[gr] = gfc->snr_increase_max[gr];
		snrinc_loLimit[gr] = 0.0;
		break_snrinc_iteration[gr] = 0;
		sav_snrinc[gr] = 0.0;
	}

    for (;;) { /* snrinc iteration to achieve target bitrate and avoid inaccurate frames for cvbr mode */

		memset(xrpow, 0, sizeof(xrpow));

	    analog_silence = VBR_new_prepare(gfc, pe, ratio, l3_xmin, frameBits, max_bits, &pad, &maximum_framebits);
		...
		max_fr = maximum_framebits;

		/* console_printf("\n>>>max_bits[0][0]: %u, max_bits[0][1]: %u, max_bits[1][0]: %u, max_bits[1][1]: %u", max_bits[0][0], max_bits[0][1], max_bits[1][0], max_bits[1][1]); */
		/* console_printf("\n>>>ns_snr_increase: %.3f", gfc->ns_snr_increase); */

		/*  quantize granules with lowest possible number of bits
		*/

		used_bits = VBR_encode_frame(gfc, const_xrpow, const_l3_xmin, const_max_bits, max_fr);

		comp_snrinc_dependence(gfc, nr_iteration, savcountBadFrames); /* use for predicting next gfc->ns_snr_increase[gr] */
	    for (gr = 0; gr < cfg->mode_gr; gr++)
			sav_snrinc[gr] = gfc->ns_snr_increase[gr]; /* use as gfc->last_snrinc[gr][0] in comp_snrinc_dependence[gr] if a new gfc->ns_snr_increase[gr] is computed for redoing the snrinc iteration */

		if ( break_on_snrinc_control(gfc, analog_silence, savcountBadFrames, break_snrinc_iteration, snrinc_hiLimit, snrinc_loLimit) ) {
			if (gfc->l3_side.tt[1][0].block_type == NORM_TYPE)
				gfc->snrinc_lastnorm = gfc->ns_snr_increase[1];
			if (gfc->l3_side.tt[0][0].block_type == STOP_TYPE) {
				gfc->lastsnrinc_stopxxx[0] = gfc->ns_snr_increase[0];
				gfc->lastsnrinc_stopxxx[1] = gfc->ns_snr_increase[1];
			}
			if (!gfc->lastFrameHasShortBlock) {
				if (gfc->hasStartShortBlock) {
					gfc->lastsnrinc_fr1_startshort[0] = gfc->ns_snr_increase[0];
					gfc->lastsnrinc_fr1_startshort[1] = gfc->ns_snr_increase[1];
				}
				if (gfc->hasShortShortBlock) {
					gfc->lastsnrinc_fr1_shortshort[0] = gfc->ns_snr_increase[0];
					gfc->lastsnrinc_fr1_shortshort[1] = gfc->ns_snr_increase[1];
				}
				if (gfc->hasShortStopBlock) {
					gfc->lastsnrinc_fr1_shortstop[0] = gfc->ns_snr_increase[0];
					gfc->lastsnrinc_fr1_shortstop[1] = gfc->ns_snr_increase[1];
				}
			}
			break;
		}

		gfc->countBadFrames = savcountBadFrames;
		memcpy(&gfc->l3_side, &sav_l3_side, sizeof(III_side_info_t)); /* gfc->l3_side = sav_l3_side; */
		gfc->countRedoneFrames += 1;
		nr_iteration++;
		for (gr = 0; gr < cfg->mode_gr; gr++) {
			gfc->last_usedBits[gr] = gfc->usedBits[gr];
			gfc->last_snrinc[gr] = sav_snrinc[gr];
		}

	} /* end of for cvbr mode varying SNR to achieve target bitrate and avoid inaccurate frames */
	...
	for (; eov->bitrate_index <= cfg->vbr_max_bitrate_index; eov->bitrate_index++) {
		if (used_bits <= frameBits[eov->bitrate_index]) {
			...
			int sav_ResvSize_Local, sav_main_data_begin_Local;
		    sav_ResvSize_Local = esv->ResvSize;
		    sav_main_data_begin_Local = l3_side->main_data_begin;
			...
			ResvFrameEnd(gfc, mean_bits);
			if ( (esv->ResvSize >= gfc->bitreservoir_target) || (eov->bitrate_index == cfg->vbr_max_bitrate_index) ) {
				done = 1;
				break;
			}
			esv->ResvSize = sav_ResvSize_Local;
			l3_side->main_data_begin = sav_main_data_begin_Local;
		}
	}

    if (gfc->frameAnalysis == 1) {
		char *LRorMS[30];
		char *blocktype1[30];
		char *blocktype2[30];
		char *extra[30];
		...
		*oogspace = "";
		*oofspace = "";
		if (gfc->l3_side.tt[0][0].block_type == NORM_TYPE)
			*blocktype1 = " norm";
		if (gfc->l3_side.tt[0][0].block_type == START_TYPE)
			*blocktype1 = "start";
		if (gfc->l3_side.tt[0][0].block_type == SHORT_TYPE)
			*blocktype1 = "short";
		if (gfc->l3_side.tt[0][0].block_type == STOP_TYPE)
			*blocktype1 = " stop";
		if (gfc->l3_side.tt[1][0].block_type == NORM_TYPE)
			*blocktype2 = "norm ";
		if (gfc->l3_side.tt[1][0].block_type == START_TYPE)
			*blocktype2 = "start";
		if (gfc->l3_side.tt[1][0].block_type == SHORT_TYPE)
			*blocktype2 = "short";
		if (gfc->l3_side.tt[1][0].block_type == STOP_TYPE)
			*blocktype2 = "stop ";
		if (gfc->ov_enc.mode_ext == 0)
			*LRorMS = "L/R";
		else
			*LRorMS = "M/S";
		if (analog_silence == 1)
			*extra = "  SILENCE";
		if (gfc->countBadFrames > savcountBadFrames) {
			if (gfc->outof_channel_space == 1)
				*oocspace = "  OUT OF CHANNEL SPACE";
			if (gfc->outof_granule_space == 1)
				*oogspace = "  OUT OF GRANULE SPACE";
			if (gfc->outof_frame_space == 1) {
				if (gfc->strictly_outof_frame_space == 1)
					*oofspace = "  STRICTLY OUT OF FRAME SPACE";
				else
					*oofspace = "  OUT OF FRAME SPACE";
			}
		}
		console_printf("\nframe#: %u  second: %.3f  %s  %s,%s   max.frameBits: %u  energy: %.4f,%.4f  targetBits: %u,%u  usedBits: %u,%u  SNR_plus: %.1f,%.1f  leftResv: %u %s%s%s%s\n"
			           , gfc->countFrames
					   , (gfc->countFrames -1.5) * 0.02612245f
					   , *LRorMS
					   , *blocktype1
					   , *blocktype2
				       , maximum_framebits
				       ...
				       , gfc->usedBits[0]
				       , gfc->usedBits[1]
				       , gfc->ns_snr_increase[0]
				       , gfc->ns_snr_increase[1]
				       , esv->ResvSize
					   , *oocspace
					   , *oogspace
					   , *oofspace
	...
	for (gr = 0; gr < cfg->mode_gr; gr++) {
		gfc->last_usedBits[gr] = gfc->usedBits[gr];
		gfc->last_snrinc[gr] = gfc->ns_snr_increase[gr];
	}
	gfc->last_Resv = esv->ResvSize;
	gfc->lastFrameHasShortBlock = gfc->hasShortBlock;
