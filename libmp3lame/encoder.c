...
{                       /* Output */
    ...
    ****************************************/

    {
        ...
        int     blocktype[2];
		FLOAT lastgr_attack_factor = 0.0f, lastgr_tonality_factor = 0.0f, gr_attack_factor = 0.0f, gr_tonality_factor = 0.0f;
		FLOAT fr_tonality_factor = 0.0, fr_attack_factor = 0.0f;
        ...
        for (gr = 0; gr < cfg->mode_gr; gr++) {
            ...
            ret = L3psycho_anal_vbr(gfc, bufp, gr,
									...
									&lastgr_attack_factor, &lastgr_tonality_factor);
            if (ret != 0)
                return -4;
			gr_tonality_factor = gfc->lastgr_tonality_factor;
			gr_attack_factor = gfc->lastgr_attack_factor;
			/* console_printf("\ngr: %i, gr_tonality_factor: %6.6f, gr_attack_factor: %6.6f", gr, gr_tonality_factor, gr_attack_factor); */
			if (gr_tonality_factor > fr_tonality_factor)
				fr_tonality_factor = gr_tonality_factor;
			if (gr_attack_factor > fr_attack_factor)
				fr_attack_factor = gr_attack_factor;
            gfc->lastgr_attack_factor = lastgr_attack_factor;	
            gfc->lastgr_tonality_factor = lastgr_tonality_factor;
		...
		gfc->fr_tonality_factor = fr_tonality_factor;
		gfc->fr_attack_factor = fr_attack_factor;
    	/* console_printf("\nfr_tonality_factor: %6.6f, fr_attack_factor: %6.6f", gfc->fr_tonality_factor, gfc->fr_attack_factor); */
   }
    ...
    ****************************************/
    ...
    for (gr = 0; gr < cfg->mode_gr; gr++) {
		gfc->energy[gr] = (tot_ener[gr][0]+tot_ener[gr][1]+tot_ener[gr][2]+tot_ener[gr][3]) / 100000000000.0;
	}
