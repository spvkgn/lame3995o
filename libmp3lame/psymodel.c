...
vbrpsy_attack_detection(lame_internal_flags * gfc, const sample_t * const buffer[2], int gr_out,
                        ...
                        int uselongblock[2], FLOAT * sum_max_ai)
{
    ...
    *sum_max_ai = 0.0f;
    ...
    for (chn = 0; chn < n_chn_psy; chn++) {
        ...
        ***************************************************************/
        ...
        {
            FLOAT   x = attack_intensity[0], y = attack_intensity[0];
            for (i = 1; i < 12; i++) {
                if (x < attack_intensity[i]) {
                    x = attack_intensity[i];
                }
                y += attack_intensity[i];
            }
			*sum_max_ai = *sum_max_ai + x;
        }
...
vbrpsy_compute_masking_l(lame_internal_flags * gfc, const FLOAT fftenergy[HBLKSIZE],
                         FLOAT eb_l[CBANDS], FLOAT thr[CBANDS], int chn, FLOAT * str_max)
{
    PsyStateVar_t *const psv = &gfc->sv_psy;
    PsyConst_CB2SB_t const *const gdl = &gfc->cd_psy->l;
    FLOAT   max[CBANDS], avg[CBANDS];
 ...
 ********************************************************************/
	...
	*str_max += str_sum;
	/* console_printf("\nchn: %i, b: %i, str_max: %6.6f", chn, b, str_max); */
    for (; b < CBANDS; ++b) {
        eb_l[b] = 0;
        thr[b] = 0;
    }
...
static void
vbrpsy_compute_masking_l_finish(lame_internal_flags * gfc, FLOAT eb_l[CBANDS], FLOAT thr[CBANDS])
{
    PsyConst_CB2SB_t const *const gdl = &gfc->cd_psy->l;
	int b;
    for (b = 0; b < gdl->npart; b++) {
...
L3psycho_anal_vbr(lame_internal_flags * gfc,
				  ...
				  FLOAT * attack_factor, FLOAT * tonality_factor)
{
    SessionConfig_t const *const cfg = &gfc->cfg;
    PsyStateVar_t *const psv = &gfc->sv_psy;
    PsyConst_CB2SB_t const *const gdl = &gfc->cd_psy->l;
	...
	FLOAT att_factor = 0.0f, ton_factor = 0.0f, str_max= 0.0f, delta_mask_adjust = 0.0f;
    ...
    vbrpsy_attack_detection(gfc, buffer, gr_out, masking_ratio, masking_MS_ratio, energy,
                            sub_short_factor, ns_attacks, uselongblock, &att_factor);
    * attack_factor = att_factor;
    ...
    /* LONG BLOCK CASE */
    {
        for (chn = 0; chn < n_chn_psy; chn++) {
            ...
            vbrpsy_compute_masking_l(gfc, fftenergy, eb[chn], thr[chn], chn, &str_max);
			ton_factor = ton_factor + str_max;
        }
		* tonality_factor = ton_factor;
		if (gfc->Qx != -9999) {
			ton_factor = 28.0f * (7.0f - att_factor) + 0.003f * ton_factor;
			ton_factor = (ton_factor - 10.0f) / 50.0f;
			if (ton_factor > 1.0f)
				ton_factor = 1.0f;
			if (ton_factor < 0.0f)
				ton_factor = 0.0f;
			ton_factor = ton_factor * ton_factor;
			* tonality_factor = ton_factor;
			delta_mask_adjust = - ton_factor * gfc->delta_maskadjust;
			if (delta_mask_adjust < gfc->maskadjust_min)
				delta_mask_adjust = gfc->maskadjust_min;
			if (gfc->enable_masking_lower == 1) {
		        gfc->sv_qnt.masking_lower = pow(10.0, (gfc->sv_qnt.mask_adjust + delta_mask_adjust) * 0.1);
			}
		}
		/* console_printf("\natt_factor: %6.6f, ton_factor: %6.6f, delta_mask_adjust: %6.6f, gfc->sv_qnt.masking_lower: %6.6f", att_factor, ton_factor, delta_mask_adjust, gfc->sv_qnt.masking_lower); */
        for (chn = 0; chn < n_chn_psy; chn++) {
            vbrpsy_compute_masking_l_finish(gfc, eb[chn], thr[chn]);
        }
		/* console_printf("\ngr_out: %i, sum_str_max: %6.6f, sum_max_ai: %6.6f", gr_out, sum_str_max, sum_max_ai); */
