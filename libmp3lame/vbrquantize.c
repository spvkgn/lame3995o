...
VBR_encode_frame(lame_internal_flags * gfc, const FLOAT xr34orig[2][2][576],
                 const FLOAT l3_xmin[2][2][SFBMAX], const int max_bits[2][2], int max_fr)
				 /* for max_bits[gr][ch] only the information max_bits[gr][ch] > 0 is relevant as a criterion for non-silence */
{
    SessionConfig_t const *const cfg = &gfc->cfg;
    ...
    int     gr, ch;
    int     ok, sum_fr;
    int     use_nbits_rfr, use_nbits_rgr;
    ...
    for (gr = 0; gr < ngr; ++gr) {
        ...
        for (ch = 0; ch < nch; ++ch) {
			...
			gfc->usedBits_ch[gr][ch] = use_nbits_ch[gr][ch];
            use_nbits_gr[gr] += use_nbits_ch[gr][ch];
        }               /* for ch */
		gfc->usedBits[gr] = use_nbits_gr[gr];
        use_nbits_fr += use_nbits_gr[gr];
    }

	/* console_printf("\nuse_nbits_ch[gr][ch]: %u/%u/%u/%u, max_bits[gr][ch]: %u/%u/%u/%u", use_nbits_ch[0][0], use_nbits_ch[0][1], use_nbits_ch[1][0], use_nbits_ch[1][1], max_bits[0][0], max_bits[0][1], max_bits[1][0], max_bits[1][1]); */

    /* check bit constrains
     */
	ok = 1;
	gfc->outof_channel_space = 0;
	gfc->outof_granule_space = 0;
	gfc->outof_frame_space = 0;
	gfc->strictly_outof_frame_space = 0;
	use_nbits_rfr = 0;
    for (gr = 0; gr < ngr; ++gr) {
		use_nbits_rgr = 0;
        ...
        for (ch = 0; ch < nch; ++ch) {
            if (use_nbits_ch[gr][ch] > MAX_BITS_PER_CHANNEL) {
                ...
                */
                ok = 0;
				gfc->outof_channel_space = 1;
				use_nbits_rgr += MAX_BITS_PER_CHANNEL;
            }
			else
				use_nbits_rgr += use_nbits_ch[gr][ch];
        }
		if (use_nbits_rgr > MAX_BITS_PER_GRANULE)
			use_nbits_rgr = MAX_BITS_PER_GRANULE;
		use_nbits_rfr += use_nbits_rgr;
	}

    if (use_nbits_fr > max_nbits_fr) {
		ok = 0;
		gfc->outof_frame_space = 1;
    }
    if (use_nbits_rfr > max_nbits_fr) {
		gfc->strictly_outof_frame_space = 1;
    }

	/* console_printf("\n>>gfc->outof_channel_space: %u, gfc->outof_granule_space: %u, gfc->outof_frame_space: %u", gfc->outof_channel_space, gfc->outof_granule_space, gfc->outof_frame_space); */
     ...
     */
    {
		gfc->countBadFrames += 1;

        ok = 1;
        sum_fr = 0;
	
		/* console_printf("\n>>>used_bits[gr][u]: %u/%u/%u/%u BAD FRAME", use_nbits_ch[0][0], use_nbits_ch[0][1], use_nbits_ch[1][0], use_nbits_ch[1][1]); */
            ...
            if (max_nbits_gr[gr] > MAX_BITS_PER_GRANULE) {
                ...
                for (ch = 0; ch < nch; ++ch) {
                    if (max_nbits_ch[gr][ch] > 0) {
                        f[ch] = sqrt(sqrt((float)max_nbits_ch[gr][ch]));
                        s += f[ch];
                    }
                    else {
                        f[ch] = 0;
		...
		/* console_printf("\n>>>sum_fr: %u, max_nbits_ch[gr][u]: %u/%u/%u/%u", sum_fr, max_nbits_ch[0][0], max_nbits_ch[0][1], max_nbits_ch[1][0], max_nbits_ch[1][1]); */
        if (sum_fr > max_nbits_fr) {
            {
                float   f[2] = {0.0f, 0.0f}, s = 0.0f;
                for (gr = 0; gr < ngr; ++gr) {
                    if (max_nbits_gr[gr] > 0) {
                        f[gr] = sqrt((float)max_nbits_gr[gr]);
                        s += f[gr];
                    }
                    else {
                        f[gr] = 0;
            ...
            for (gr = 0; gr < ngr; ++gr) {
                ...
                for (ch = 0; ch < nch; ++ch) {
                    if (max_nbits_ch[gr][ch] > 0) {
                        f[ch] = sqrt((float)max_nbits_ch[gr][ch]);
                        s += f[ch];
                    }
                    else {
                        f[ch] = 0;
	...
	/* console_printf("\n>>>used_bits[gr][u]: %u/%u/%u/%u, use_nbits_gr: %u/%u, use_nbits_fr: %u BAD FRAME REMOVED", use_nbits_ch[0][0], use_nbits_ch[0][1], use_nbits_ch[1][0], use_nbits_ch[1][1], use_nbits_gr[0], use_nbits_gr[1], use_nbits_fr); */
