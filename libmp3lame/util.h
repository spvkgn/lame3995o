...
#  define  LAME_ID   0xFFF88E3B
		...
		float	Vx, Qx;
		int		frameAnalysis;
		double  ns_snr_increase[2]; /* for each granule */
		double  snr_increase_max[2]; /* for each granule */
		double  snrinc_lo[2], snrinc_hi[2]; 
		int		bits_lo[2], bits_hi[2];
		float   energy[2];
		int     countFrames, countBadFrames, countRedoneFrames;
		float   lastgr_attack_factor, lastgr_tonality_factor, fr_tonality_factor, fr_attack_factor, delta_maskadjust, maskadjust_min;
		int		enable_masking_lower;
		int		size_bitreservoir; /* bit reservoir size to keep */
		double	adbr_norm_ton, adbr_norm_att, adbr_norm_min; /* audio data bitrate target for normal blocks */
		double	adbr_norm_max; /* maximum energy driven correction term of adbr_norm */
		double	adbr_short_ton, adbr_short_att, adbr_short_min; /* audio data bitrate target [kbps] for a granule of short blocktype */
		double	adbr_start_ton, adbr_start_att, adbr_start_min; /* audio data bitrate for a granule of start type */
		double	adbr_stop_ton, adbr_stop_att, adbr_stop_min; /* audio data bitrate for a granule of stop type */
		double	adbr_tolerance; /* tolerance bitrate [kbps] to be allowed above target bitrate */
		double	snrinc_lastnorm; /* SNR increase for the last normal block granule */
		double	snrincdep_lastnorm; /* SNR increase dependence for the last normal block granule */
		double	snrincmax_norm; /* maximum SNR increase for a normal block granule */
		double	snrincmax_short; /* maximum SNR increase for a short block granule */
		double	snrincmax_start; /* maximum SNR increase for a start block granule */
		double	snrincmax_stop; /* maximum SNR increase for a stop block granule */
		double	insane_factor; /* factor between 0 and 1 for going for insane long block bitrates (0: going not insane at all, 1: going totally insane) */
		int		bitreservoir_target; /* target bitreservoir size */
		int		targetBits_ori_norm_ton, targetBits_ori_norm_att, targetBits_ori_norm_min, targetBits_ori_norm_max;
		int		targetBits_ori_short_ton, targetBits_ori_short_att, targetBits_ori_short_min;
		int		targetBits_ori_start_ton, targetBits_ori_start_att, targetBits_ori_start_min;
		int		targetBits_ori_stop_ton, targetBits_ori_stop_att, targetBits_ori_stop_min, targetBits_ori_tolerance;
		int		targetBits[2]; /* targetBits for each granule */
		int		hasShortBlock; /* 1: one or both of the two granules is of blocktype 'SHORT', 0: otherwise */
		int		hasStartShortBlock, hasShortShortBlock, hasShortStopBlock; /* detailed blocktype combination for a short block frame */
		double	lastsnrinc_stopxxx[2], lastsnrinc_fr1_startshort[2], lastsnrinc_fr1_shortshort[2], lastsnrinc_fr1_shortstop[2];
		int		lastFrameHasShortBlock;
		int		outof_channel_space, outof_granule_space, outof_frame_space, strictly_outof_frame_space;
		int		usedBits[2], last_usedBits[2]; /* for each granule */
		int		usedBits_ch[2][2]; /* for each channel */
		double	snrinc_dependence[2], snrinc_depref[2], last_snrinc[2]; /* for each granule */
		int		bits_depref[2];
		double	last_Resv;
...
***********************************************************************/
	...
	void CDECL set_qntFactors(lame_internal_flags *, int gr);
