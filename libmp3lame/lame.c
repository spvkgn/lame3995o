...
lame_init_params(lame_global_flags * gfp)
{
   ...
   *******************************************************/
    ...
    case vbr_mtrh:{
			...
			/*
			if ( (gfc->VxMode == '+') && (gfc->Vx < 1.0f) ) {
				cfg->ATHfixpoint += ( 2.0f - 2.0f * gfp->VBR_q );
				gfp->maskingadjust -= ( 0.8f - 0.8f * gfp->VBR_q );
			}
			*/
...
lame_print_internals(const lame_global_flags * gfp)
{
	...
	MSGF(gfc, "\texperimental psy tunings by Naoki Shibata\n");
    MSGF(gfc, "\t   adjust masking bass long / short = %.3f / %.3f dB\n",
	     10 * log10(gfc->sv_qnt.longfact[0]), 10 * log10(gfc->sv_qnt.shortfact[0]));
	MSGF(gfc, "\t   adjust masking alto long / short = %.3f / %.3f dB\n",
		 10 * log10(gfc->sv_qnt.longfact[7]), 10 * log10(gfc->sv_qnt.shortfact[3]));
	MSGF(gfc, "\t   adjust masking treble long / short = %.3f / %.3f dB\n",
		 10 * log10(gfc->sv_qnt.longfact[14]), 10 * log10(gfc->sv_qnt.shortfact[7]));
	MSGF(gfc, "\t   adjust masking sfb21 long / sfb12 short = %.3f / %.3f dB\n",
		 10 * log10(gfc->sv_qnt.longfact[21]), 10 * log10(gfc->sv_qnt.shortfact[12]));
...
lame_init_old(lame_global_flags * gfp)
{
	...
	lame_set_Vx(gfp, -9999);
	lame_set_Qx(gfp, -9999);
	lame_set_adbr_norm_ton(gfp, -9999.9);
	lame_set_adbr_norm_att(gfp, -9999.9);
	lame_set_adbr_norm_min(gfp, -9999.9);
	lame_set_adbr_short_ton(gfp, -9999.9);
	lame_set_adbr_short_att(gfp, -9999.9);
	lame_set_adbr_short_min(gfp, -9999.9);
	lame_set_adbr_start_ton(gfp, -9999.9);
	lame_set_adbr_start_att(gfp, -9999.9);
	lame_set_adbr_start_min(gfp, -9999.9);
	lame_set_adbr_stop_ton(gfp, -9999.9);
	lame_set_adbr_stop_att(gfp, -9999.9);
	lame_set_adbr_stop_min(gfp, -9999.9);
	lame_set_insane_factor(gfp, -9999.9);
	lame_set_frameAnalysis(gfp, 0);
	gfc->enable_masking_lower = 0;
