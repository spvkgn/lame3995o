...
int
short_help(FILE * const fp, const char *ProgramName)
...
int
long_help(FILE * const fp, const char *ProgramName, int lessmode)
{                       /* print long syntax help */
    ...
    fprintf(fp,
            ...
            "\n"
            "    <infile> and/or <outfile> can be \"-\", which means stdin/stdout.\n"
            "\n" "RECOMMENDED:\n" "    lame3995o -Q2 input.wav output.mp3\n" "\n", ProgramName);
    fprintf(fp,
            "OPTIONS:\n"
            "  Input options:\n"
    ...
    fprintf(fp,
            ...
            "    --frameAnalysis print detailed frame analysis\n"
            "\n"
			);
    fprintf(fp,
    ...
    fprintf(fp,
            "  VBR options:\n"
			"    -V x            sets quality for the standard VBR mode\n"
			"    -Q x            (x <= 3) sets quality for an alternative VBR mode which\n"
            "                    focusses on difficult samples\n"
...
double
Q2Vlevel(double Qlevel) {

	double	const Q_level[9] = { 4.0, 3.5, 3.0, 2.5, 2.0, 1.7, 1.0, 0.5, 0.0 };
	double	const V_level[9] = { 4.9, 4.4, 4.5, 4.0, 3.5, 3.0, 3.0, 2.0, 0.0 };
	int		i, prev_i;
	double	share;
	...
	if (i == 0) {
		...
		prev_i = i - 1;
		share = (Q_level[i] - Qlevel) / (Q_level[i] - Q_level[prev_i]);
	}
	return ( V_level[i] + share * (V_level[prev_i] - V_level[i]) );
}
...
static int evaluateArgument(char const* token, char const* arg, char* _EndPtr)
{
    if (arg != 0 && arg != _EndPtr)
        return 1;
    error_printf("argument missing for '%s'\n", token);
    return 0;
}
    ...
    for (i = 0; ++i < argc;) {
        ...
        if (*token++ == '-') {
            ...
            if (*token == '-') { /* GNU style */
                ...
                T_ELIF("frameAnalysis")
                    global_ui_config.silent = 15;
				    lame_set_frameAnalysis(gfp, 1);
                ...
                T_ELIF2("help", "usage")
                    if (0 == local_strncasecmp(nextArg, "id3", 3)) {
                        ...
                        short_help(stdout, ProgramName);
                    }
                return -1;

                T_ELIF("longhelp")
                    long_help(stdout, ProgramName, 0 /* lessmode=NO */ );
                return -1;

                T_ELIF("?")
#ifdef __unix__
                    FILE   *fp = popen("less -Mqc", "w");
                    long_help(fp, ProgramName, 0 /* lessmode=NO */ );
                    pclose(fp);
#else
                    long_help(stdout, ProgramName, 1 /* lessmode=YES */ );
#endif
                return -1;
                ...
                while ((c = *token++) != '\0') {
                    ...
                    case 'V':
						{
							...
							for (i=0;;i++) {
								...
								if (arg[i] == '+') {
									if (arg[i+1] == '\0') {
					                    error_printf("-Vn+ is not a valid option\n");
						                return -1;
									}
								}
							}
		                    ...
		                    if (argUsed) {
								if (lame_get_Qx(gfp) != -9999) {
					                error_printf("-Vx not allowed together with -Qx.\n");
								    return -1;
								}
					...
					case 'v':
                        ...
                        if (lame_get_VBR(gfp) == vbr_off)
                            lame_set_VBR(gfp, vbr_default);
                        lame_set_VBR_quality(gfp, vbr_default);
                        break;
					...
					case 'Q':
						{
		                    ...
		                    if (argUsed) {
								...
								if (double_value > 3.0)
									double_value = 3.0;
								lame_set_Qx(gfp, double_value);
								double_value = Q2Vlevel(double_value);
								/* to change VBR default look in lame.h */
								if (lame_get_VBR(gfp) == vbr_off)
									lame_set_VBR(gfp, vbr_default);
	                            lame_set_VBR_quality(gfp, (float) double_value);
